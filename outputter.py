...
# Thread events to kill the infinite loops:
kill_init = threading.Event()
kill_grid = threading.Event()
...
def init_sequence():
    led_seq = [(0, 0), (3, 0), (3, 3), (0, 3)]

    while not kill_init.wait(0.1):
        for x, y in led_seq:
            if not kill_init.wait(0.1):
                led_values = led_map[(x, y)]
